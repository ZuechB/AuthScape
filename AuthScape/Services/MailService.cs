...
namespace Services
{
    ...
    {
        Task InviteUser(InviteRequest inviteRequest, AppUser user, string PasswordResetToken);
    ...
    public class MailService : IMailService
    {
        ...
        public async Task InviteUser(InviteRequest inviteRequest, AppUser user, string PasswordResetToken)
        {
            if (inviteRequest.CompanyId != null)
            {
                var company = await databaseContext.Companies.Where(c => c.Id == inviteRequest.CompanyId.Value).FirstOrDefaultAsync();
                if (company != null)
                {
